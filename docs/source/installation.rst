...
+ **Windows**: download the `installation file <https://raw.githubusercontent.com/harmslab/pytc-gui/master/pytc_install/pytc-gui_v1.0_setup.exe?raw=true>`_ and follow the prompts for the installer. 
+ **Mac**: download the `dmg <https://github.com/hrmyd/pytc-gui/blob/master/pytc_install/pytc_osx.dmg?raw=true>`_, unpack it, and then drag the pytc icon into the Applications folder.  
